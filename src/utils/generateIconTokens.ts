...
import { ConfigPlugin, transform as svgrTransform } from "@svgr/core";
...
export default async function generateIconTokens(): Promise<void> {
  ...
  const webOutput = fileURLToPath(new URL("../../assets/web/icons/", import.meta.url));
  ...
  for (const icon of icons) {
    ...
    const esm = await svgrTransform(svg, {
      plugins: [
        svgrPluginJsx as unknown as ConfigPlugin,
      ],
      icon: true,
    ...
    const cjs = await svgrTransform(svg, {
      plugins: [
        svgrPluginJsx as unknown as ConfigPlugin,
      ],
      icon: true,
    ...
    await fs.writeFile(path.join(webOutput, `${parsedPath.name}.js`), esm, "utf-8");
    await fs.writeFile(path.join(webOutput, `${parsedPath.name}.cjs`), cjs, "utf-8");
...
`;
    ...
    await fs.writeFile(path.join(webOutput, `${parsedPath.name}.d.ts`), dTs, "utf-8");
    ...
    indexEsmStatements.push(
      t.exportNamedDeclaration(
        null,
        [t.exportSpecifier(t.identifier("default"), t.identifier(componentName))],
    ...
    indexCjsExportsProperties.push(
      t.objectProperty(
        ...
        t.callExpression(t.identifier("require"), [t.stringLiteral(`./${parsedPath.name}.cjs`)])
      ),
    );
  }
